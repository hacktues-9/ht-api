create function searchuser(search character varying, teamid bigint, uid bigint)
    returns TABLE
            (
                id              bigint,
                name            text,
...
BEGIN
    RETURN QUERY
        SELECT u.id,
               ...
               s.profile_picture,
               inv.id IS NOT NULL                       AS isInvited
        FROM users u
...
alter function searchuser(varchar, bigint, bigint) owner to doadmin;
