...
func ResendVerification(w http.ResponseWriter, r *http.Request, db *gorm.DB) {
	...
	if user.ID == 0 {
		...
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusNotFound, "user not found", 0), errors.New("user not found"), http.StatusNotFound, "AdminResendVerification")
		return
	}
	...
	if security.ID == 0 {
		...
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusNotFound, "security not found", 0), errors.New("user not found"), http.StatusNotFound, "AdminResendVerification")
		return
	}

	if security.EmailVerified {
		fmt.Printf("[ ERROR ] [ ResendVerification ] email already verified: %v\n", errors.New("user not found"))
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusBadRequest, "email already verified", 0), errors.New("user not found"), http.StatusBadRequest, "AdminResendVerification")
		return
	}
...
func ResendVerificationElsys(w http.ResponseWriter, r *http.Request, db *gorm.DB) {
	...
	db.Table("users").Where("id = ?", id).Find(&user)
	if user.ID == 0 {
		fmt.Printf("[ ERROR ] [ ResendVerificationElsys ] user not found \n")
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusNotFound, "user not found", 0), errors.New("user not found"), http.StatusNotFound, "AdminResendVerificationElsys")
		return
	}
	...
	db.Table("security").Where("id = ?", user.SecurityID).Find(&security)
	if security.ID == 0 {
		fmt.Printf("[ ERROR ] [ ResendVerificationElsys ] security not found\n")
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusNotFound, "security not found", 0), errors.New("user not found"), http.StatusNotFound, "AdminResendVerificationElsys")
		return
	}

	if security.ElsysEmailVerified {
		fmt.Printf("[ ERROR ] [ ResendVerificationElsys ] email already verified\n")
		models.RespHandler(w, r, models.DefaultNegResponse(http.StatusBadRequest, "email already verified", 0), errors.New("user not found"), http.StatusBadRequest, "AdminResendVerificationElsys")
		return
	}
