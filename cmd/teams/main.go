...
func GetTeam(w http.ResponseWriter, r *http.Request, db *gorm.DB) {
	...
	db.Table("users").Select("users.id, concat(users.first_name, ' ', users.last_name) AS name, socials.profile_picture AS avatar, role.name AS role").Joins("JOIN info ON users.info_id = info.id").Joins("JOIN socials ON info.socials_id = socials.id").Joins("JOIN role ON role.id = users.role_id").Where("users.team_id = ? AND deleted_at IS NULL", teamID).Scan(&team.Members)
