...
func GetTeam(w http.ResponseWriter, r *http.Request, db *gorm.DB) {
	...
	db.Table("users").Select("users.id, concat(users.first_name, ' ', users.last_name) AS name, socials.profile_picture AS avatar, role.name AS role, concat(grade, class.name) as class").Joins("JOIN info ON users.info_id = info.id").Joins("JOIN class ON info.class_id = class.id").Joins("JOIN socials ON info.socials_id = socials.id").Joins("JOIN role ON role.id = users.role_id").Where("users.team_id = ? AND users.deleted_at IS NULL", teamID).Order("role_id desc").Scan(&team.Members)
